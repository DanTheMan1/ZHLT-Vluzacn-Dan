...
void GetFaceExtents (int facenum, int mins_out[2], int maxs_out[2])
{
	...
	for (i = 0; i < f->numedges; i++)
	{
		...
		for (j = 0; j < 2; j++)
		{
#if defined(VERSION_32BIT) && 1200 <= _MSC_VER && _MSC_VER <= 1600 && _M_IX86_FP == 0
			// When compiled for x86 under MSVC (prior to VS 11), the intermediate values are stored as 64-bit double by default.
			// See this article for details: http://www.altdevblogaday.com/2012/03/22/intermediate-floating-point-precision/
			val = v->point[0] * tex->vecs[j][0] +
				  v->point[1] * tex->vecs[j][1] +
				  v->point[2] * tex->vecs[j][2] +
				  tex->vecs[j][3];
#else
			// Note that 64-bit processor always use 80-bit internal precision. You must rewrite this if you want GetFaceExtents to support x64.
...
{
#ifdef ZHLT_64BIT_FIX
#if !defined (CAN_CALC_EXTENT) && !defined (HLRAD)
	...
	for (k = 0; k < g_numfaces; k++)
	{
		...
		if (extents[0] < 0 || extents[1] < 0 || extents[0] > max (512, MAX_SURFACE_EXTENT * TEXTURE_STEP) || extents[1] > max (512, MAX_SURFACE_EXTENT * TEXTURE_STEP))
	...
	// this function needs to be as stable as possible because it might be called from ripent
{
	...
	for (linestart = 0; linestart < g_entdatasize; )
	{
		...
		else
		{
			...
			if (quotes[1] - (quotes[0] + 1) == strlen ("wad") && !strncmp (&g_dentdata[quotes[0] + 1], "wad", strlen ("wad")))
			{
...
void DeleteEmbeddedLightmaps ()
{
	...
	int *texture_originals = (int *)malloc (numtextures * sizeof (int));
	hlassume (texture_originals != NULL, assume_NoMemory);
	for (i = 0; i < numtextures; i++)
	{
		texture_originals[i] = -1;
		...
		if (strlen (name) >= 6 && !strnicmp (&name[1], "_rad", 4) && '0' <= name[5] && name[5] <= '9')
		{
			texture_originals[i] = atoi (&name[5]);
			if (texture_originals[i] < 0 || texture_originals[i] >= g_numtexinfo)
			{
				Warning ("Invalid index of original texinfo: %d parsed from texture name '%s'", texture_originals[i], name);
				texture_originals[i] = -1;
			}
		}
	}
	for (i = 0; i < g_numfaces; i++)
	{
		...
		texinfo_t *info = &g_texinfo[f->texinfo];
		if (info->miptex < 0 || info->miptex >= numtextures)
		{
			continue;
		}
		if (texture_originals[info->miptex] != -1)
		{
			f->texinfo = texture_originals[info->miptex];
			countrestoredfaces++;
		}
	}
	free (texture_originals);
	...
	{
		if (textureused[i] || texture_originals[i] == -1)
		{
