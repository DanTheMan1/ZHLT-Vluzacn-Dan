...
void            WriteLog(const char* const message)
{
    ...
    /*fprintf(stdout, message);*/ fprintf(stdout, "%s", message); //--vluzacn
    fflush(stdout);
#ifdef ZHLT_CONSOLE
	if (twice)
	{
...
void CDECL PrintConsole(const char* const warning, ...)
{
    char            message[MAX_MESSAGE];
...
void LoadLangFile (const char *name, const char *argv0)
{
	...
	if (!f)
	{
		ExtractFilePath (argv0, filepath);
